...
int64_t first_free_inode(){
    ...
    for(i = 0; i <= INODES_PER_BLOCK; i++){
        ...
        if(i == INODES_PER_BLOCK){
            i = 0;
            &inode_block += sizeof(block__t);
            inode_block_offset += 1;
        }

        inode = inode_block.inodes[i];
...
inode_t find_inode(uint64_t inumber){
    ...
    &inode_block += inode_block_offset * sizeof(block__t);

    inode__t inode = inode_block.inodes[i - inode_block_offset * INODES_PER_BLOCK];
